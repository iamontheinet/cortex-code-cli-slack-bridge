---
name: slack-bridge
description: "Bidirectional Slack DM bridge for remote notifications and confirmations. Use when: user says enable slack, start slack, activate slack, slack on, /slack, disable slack, stop slack, slack off. Also use for: sending notifications to user's phone, requesting remote approval, checking for Slack replies. Triggers: enable slack, start slack, activate slack, slack on, /slack, disable slack, stop slack, deactivate slack, slack off, notify, bridge, remote, phone, DM, confirm remotely."
...
"Slack inbox check: Read ~/.cortex-slack-bridge/inbox_${CORTEX_SESSION_ID}.json. If missing or [], sleep 30 seconds, then read again. If still empty, do nothing — completely silent. If entries found on either read, process them."
```
